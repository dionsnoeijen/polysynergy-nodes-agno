...
def find_connected_db_settings(node: Node, for_team: bool = False) -> dict:
    """Find and return storage settings from connected storage node."""
    storage_connections = [c for c in node.get_in_connections() if c.target_handle == "db"]

    settings = None
    for conn in storage_connections:
        storage_node = node.state.get_node_by_id(conn.source_node_id)
        if hasattr(storage_node, "provide_db_settings"):
            settings = storage_node.provide_db_settings()
            break
