...
from polysynergy_nodes_agno.agent.utils.find_connected_members import find_connected_members
...
from polysynergy_nodes_agno.agent.utils.send_chat_stream_event import send_chat_stream_event
...
class AgnoTeam(ServiceNode):
    ...
    async def _setup(self):
        ...
        return model, settings, debug_level, mode, tool_info_list, member_info_list

    async def _create_team(self):
        model, settings, debug_level, mode, tool_info_list, member_info_list = await self._setup()
        ...
        print('MEMBERS', member_instances)
        ...
        self.instance = Team(
            name=self.team_name,
            members=member_instances or None,
    ...
    async def execute(self):
        ...
        if has_connected_agent_or_team(self):
            print('SKIPPED TEAM EXECUTION: connected agent or team found')
            return
        ...
        print('MEMBER ID TO NODE ID', self.map_member_id_to_node_id)
        ...
        try:
            ...
            async def _collect_response(generator):
                ...
                async for step in generator:
                    if step.event == "RunResponseContent" or step.event == "TeamRunResponseContent" or step.event == "TeamToolCallCompleted":
                        node_id = (
